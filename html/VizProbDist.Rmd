---
title: "Vizualizing Probability Distributions"
...
gbinom = function(n, p, low=0, high=n,scale = F, a=NA,b=NA,calcProb=!all(is.na(c(a,b))),quantile=NA,calcQuant=!is.na(quantile))
{
  ...
  plot(c(low,high), c(0,max(probs)), type = "n", xlab = "Binomial Outcomes",
  ...
  if(calcProb) {
    ...
    text(paste("P(",a," <= Y <= ",b,") = ",round(prob,6),sep=""),line=0,col.main=4)
...
```{r}
gbinom(10, 0.8, a = 8, b = 10, scale = T)

```

```{r}
# Input: n = sample size, N = population size, X = population successes, 
# low = minimum occurrences, high = maximum occurrences, 
# a = lower threshold, b = upper threshold, quantile = quantile
ghyper= function(n, N, X, low=0, high=n, scale = F, a=NA, b=NA, calcProb=!all(is.na(c(a,b))), quantile=NA, calcQuant=!is.na(quantile))
{
  mean <- n * X/N
  sd <-  sqrt(n * (X/N) * ((N-X)/N) * ((N-n)/(N-1)))
  if(scale && (n > 10)) {
	low <-  max(0, round(mean - 4 * sd))
	#low <- max(0, n+X-N)
  high <-  min(n, round(mean + 4 * sd))
  }
  values <-  low:high
  probs <-  dhyper(values, X, N-X, n)
  plot(c(low,high), c(0,max(probs)), type = "n", xlab = "Hypergeometric Outcomes",
      ylab = "Probability Density",
      main = paste("Hypergeometric Distribution \n", "N =", N, ", X =", X, ", n =", n))
  lines(values, probs, type = "h", col = 2)
  abline(h=0,col=3)
  if(calcProb) {
    if(is.na(a))
    ...
    a = round(a)
    b = round(b)
    prob <-  dhyper(b, X, N-X, n) - dhyper(a-1, X, N-X, n)
    text(paste("P(",a," <= x <= ",b,") = ",round(prob,6),sep=""),col.main=4)
    u <-  seq(max(c(a,low)),min(c(b,high)),by=1)
    v <-  dhyper(u, X, N-X, n)
    lines(u,v,type="h",col=4)
  }
    ...
    if(quantile < 0 || quantile > 1)
      stop("quantile must be between 0 and 1")
    x <-  qhyper(quantile, X, N-X, n)
    title(paste("The ",quantile," quantile = ",x,sep=""),col.main=4)
    u <-  0:x
    v <- dhyper(u, X, N-X, n)
    lines(u,v,type="h",col=4)
  }
