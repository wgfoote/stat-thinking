---
title: 'Yet Another mockFINAL -- BUAN 227'
output: 
  html_document:
    toc: true
---
...
description <- "a new rare earth commodity"
n <- 6
m_x <- 16
s_x <- 33
s_e <- 5
...
Y <- abs(14 - 0.8 * X + rnorm(n, sd = s_e))
Y_0 <- abs(16 - 0.75 * X + rnorm(n, sd = s_e*1.10))
...
```{r}
#
header <- c("east price", "west price", "quantity")
margin <- 1:N
col_0 <- Y_0
col_1 <- Y
col_2 <- X
...
Producers, traders, and consumers price the commodity in two different global regions.

Here are calculations for the `r name_Y` andd `r name_X` data:
...
We are going to try to forecast prices. There are two prices on the planet. Do they act as if one world price? Formulate an hypothesis to test a claim that there is one world price. Test the hypothesis and interpret the results.
...
1. The analyst formulates the null hypothesis that the mean price in region is the same for both regions, and thus their difference is zero, or as
$$
H_0: \mu_1 - \mu_2 = 0
$$
