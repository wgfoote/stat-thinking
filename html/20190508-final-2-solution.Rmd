...
5. A $t$ score of `r round(t_diff, 2)` means that the score, and the difference between the means, is in the `r hypo_diff`.

- There is at most a 5\% chance that management is wrong in its assertion that the prices are the same or different.

- Another interpretation is possible. Using Excel the analyst can compute `= T.DIST( `r t_diff`, `r n0+n-2`)` to calculate `r p_diff`. When the t-stat is less than zero, use `T.DIST()`, else use `1 = T.DIST()`. This is the p-value which is the cumulative probability less than (or greater if positive) than $t=$`r t_diff` that she was wrong about the rejection of the null hypothesis. Since the p-value is greater than `r alpha/2 * 100`\% tolerated, she `accepts the null hpythesis`r hypo_diff` as probably true.

- The analyst compares the `p-value` with the significance level of 5\% and sees that there is a stronger chance of a Type I false negative error than indicated simply by looking at the rejection region. The analyst then `r hypo_diff` that there one world price for this new rare earth commodity.
