---
title: "What types of data are there?"
...
- Cases: or "observations" include measurements relative to 143,196 births between the years 1989 and 1993.

- Variables: air quality station (nominal), year (nominal) length of gestation (continuous), levels of carbon monoxide were recorded in parts per million (continuous), nitrogen dioxide and ozone in parts per hundred million (continuous), and coarse particulate matter (PM10) in $\mu / m^3$ (continuous).

What further questions might be possibly related to these measures? Do these questions anticipate the collection of discrete or ordinal variables?
...
a. Relevance and Selection: Does the hierarchy use terms and relationships that hint at attributes in the data in the example? Does the hierarchy anticipate, and possibly raise further questions about the data? Does the hierarchy select relevant data and de-select irrevelant data.

b. sufficiency: If you use another node or relationship in the hierarchy, does that add anything new to the description that is relevant?

c. Parsimony: is the hierarchical description of data itself verifiable in the data?

d. Completeness: does this hierarchy account for all of the aspects you observe about data in this example? In other words, does the hierarchy somehow employ all of the data available, relevant, sufficient, and verifiable in this example.
</br>
...
Yes it does. Anticipated are variables that are non-integer (as well as integer) and simply unordered categorical data. Other types are possible, but not present.

b. sufficiency: If you use another node or relationship in the hierarchy, does that add anything new to the description that is relevant?

No it does not. It seems that the continuous and discontinuous numeric variables are sufficient for numeric variables if only because they are mutually exclusive. The same for nominal (unordered) and ordinal (ordered). If fact the hierarchy _overfits_ the data in this exercise by the inclusion of discrete and ordinal data types. Including these anticipate relevancy and selection in other studies. 

c. Parsimony: is the hierarchical description of data itself verifiable in the data?

Yes it does. Again our data seems well matched to the lowest level of the hierarchy. Further levels might delineate attributes of the previous level. For example, add another component, say integer. But integer is just another way of saying discrete variable. so this doesn't add anything. Are there other categories of numeric or categorical data? There do not seem so.

d. Completeness: does this hierarchy account for all of the aspects you observe about data in this example? In other words, does the hierarchy somehow employ all of the data available, relevant, sufficient, and verifiable in this example.

Yes it does. The data in the example are continuous, discrete, nominal, and ordinal. There does not seem to be any other relevant data available within the field of this example.
